          ...
          [...history].reverse().map((turn, idx) => (
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-tighter">Turn {turn.turnNumber}</span>
                <span className={`flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded uppercase font-syncopate
                  ${turn.movement > 0 ? 'bg-yellow-500/10 text-yellow-500' : turn.movement < 0 ? 'bg-red-500/10 text-red-500' : 'bg-zinc-800 text-zinc-400'}
                `}>
              ...
              <p className="text-xs text-zinc-300 mb-2 line-clamp-2 italic leading-relaxed">"{turn.dilemma}"</p>
              
              {turn.gitaVerse && (
                <div className="mt-2 pt-2 border-t border-zinc-800 flex items-start gap-2">
                  <Quote className="w-3 h-3 text-yellow-500/40 shrink-0 mt-0.5" />
                  <p className="text-[10px] text-zinc-500 italic line-clamp-2 leading-tight">
                    {turn.gitaVerse}
                  </p>
                </div>
              )}
              ...
              <div className="mt-3 flex items-center justify-between border-t border-zinc-800/50 pt-2">
                <span className="text-[9px] font-bold text-zinc-600 uppercase font-syncopate tracking-widest">
                  Points: <span className={turn.karmaDelta > 0 ? 'text-yellow-500' : 'text-red-500'}>{turn.karmaDelta > 0 ? '+' : ''}{turn.karmaDelta}</span>
                </span>
                <span className="text-[9px] font-mono text-zinc-700 font-bold">TILE {turn.tile}</span>
              </div>
            </div>
          ))
        )}
