      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-4">
        {history.length === 0 ? (
          <div className="h-full flex items-center justify-center text-zinc-700 text-sm text-center px-4 italic">
            Play a turn to see what happens...
          </div>
        ) : (
          [...history].reverse().map((turn, idx) => (
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <span className="text-[10px] font-mono text-zinc-500">MOVE {turn.turnNumber}</span>
              ...
              <p className="text-[10px] text-zinc-500 font-medium">Result: <span className="text-zinc-400">{turn.feedback}</span></p>
              <div className="mt-2 flex items-center justify-between">
                <span className="text-[10px] font-bold text-zinc-500">POINTS: <span className={turn.karmaDelta > 0 ? 'text-yellow-500' : 'text-emerald-500'}>{turn.karmaDelta > 0 ? '+' : ''}{turn.karmaDelta}</span></span>
                <span className="text-[10px] font-mono text-zinc-600">SQUARE {turn.tile}</span>
              </div>
            </div>
          ))
        )}
